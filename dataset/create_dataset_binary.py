...
for etf in etfList:
    ...
    # # Add the moving average column to 'data'
    # data['SMA'] = data['Close'].rolling(window=window_size).mean()

    # # Prepare your label list
    # labelList = []

    # # Iterate through the data to assign labels
    # print("range of leng")
    # print(len(range(window_size, len(data) - 1)))
    # for i in range(window_size, len(data) - 1):  # Start from window_size to avoid NaNs in SMA
    #     closePriceDifference = data['Close'].iloc[i + 1] - data['Close'].iloc[i]

    #     # If the price has increased enough to pass the threshold
    #     if closePriceDifference > thresholdPrice:
    #         labelList.append(0.0)  # BUY

    #     # If the price has decreased enough to pass the threshold
    #     elif closePriceDifference < -thresholdPrice:
    #         labelList.append(1.0)  # SELL

    #     # If the change is exactly the threshold, decide based on trend
    #     elif abs(closePriceDifference) == thresholdPrice:
    #         if data['Close'].iloc[i] > data['SMA'].iloc[i]:
    #             labelList.append(0.0)  # BUY for upward trend
    #         else:
    #             labelList.append(1.0)  # SELL for downward trend


    def label_price_data(data, threshold=0.01):
        """
        Labels price data as Buy (1) or Sell (0) based on the threshold.

        :param data: Pandas Series or list containing the price data.
        :param threshold: Percentage change to determine Buy or Sell.
        ...
        for i in range(1, len(data)):
            ...
            # Label as 'Buy' if the change is greater than or equal to the threshold
            if price_change >= threshold:
                labels.append(1)  # Buy
            # Label as 'Sell' otherwise
            else:
                labels.append(0)  # Sell

        return np.array(labels)
    
    def find_balanced_threshold(data, start=0.0001, end=0.5, step=0.001):
        best_threshold = start
        best_balance = float('inf')

        for threshold in np.arange(start, end, step):
            labels = label_price_data(data, threshold)
            balance = abs(sum(labels) - (len(labels) - sum(labels)))
            if balance < best_balance:
                best_balance = balance
                best_threshold = threshold

        return best_threshold
    # balanced_threshold = find_balanced_threshold(data_close)
    # print("Balanced Threshold:", balanced_threshold)
    print(f"shape of data close: {data_close.shape}")
    print(f"shape of img: {np.array(imageList).shape}")
    labels = label_price_data(data_close, 0.00001)
    print(labels.shape)
    labelList = labels
    unique, counts = np.unique(labels, return_counts=True)
    label_distribution = dict(zip(unique, counts))
    print(label_distribution)
    contains_nan = np.isnan(labelList).any()

    print(f"Does labelList contain NaN values? {contains_nan}")
    # Buy : 0
    # Hold: 1
    # Sell: 2
    # for i in range(window_size, len(data_close)-1):  # Start from window_size to avoid NaNs
    ...
    #     # If the price has decreased enough to pass the threshold
    #     elif(closePriceDifference < -thresholdPrice):
    #         labelList.append(np.array([1.0]))  # SELL

    #     # If the change is exactly the threshold, decide based on trend
    #     elif(abs(closePriceDifference) == thresholdPrice):
    #         if data_close.iloc[i] > data_close['SMA'].iloc[i]:
    #             labelList.append(np.array([0.0]))  # BUY for upward trend
    #         else:
    #             labelList.append(np.array([1.0]))  # SELL for downward trend
    ...
    np.save(f"/home/emir/Desktop/dev/datasets/ETF_new/TrainData/x_{etf}.npy", x_train)
    np.save(f"/home/emir/Desktop/dev/datasets/ETF_new/TrainData/y_{etf}.npy", y_train)
    np.save(f"/home/emir/Desktop/dev/datasets/ETF_new/TestData/x_{etf}.npy", x_test)
    np.save(f"/home/emir/Desktop/dev/datasets/ETF_new/TestData/y_{etf}.npy", y_test)
